...
# Промо-ответ на вопрос про БЕСПЛАТНЫЕ розы/цветы (акция новой точки). Отдаём
# детерминированно, одинаково всем — мимо LLM, чтобы текст не «плыл».
FREE_ROSES_PROMO = (
    "Да, конечно! 🌹\n\n"
    "Мы открыли новую точку «ЦветоМир» на ул. Машиностроительная, 8/1 — "
    "и по этому поводу дарим вам розы!\n\n"
    "Чтобы узнать по наличию, позвоните по номеру:\n"
    "+7 917 777-31-71\n\n"
    "Ждём вашего звонка!"
)
# «бесплатные розы/розочки/цветы/букет», «раздаёте/дарите розы бесплатно» и т.п.
_FREE_ROSES_RE = re.compile(
    r"(беспл|халяв|даром|бесплатн\w*)[^\n]{0,40}(роз|цвет|букет)"
    r"|(роз|цвет|букет)\w*[^\n]{0,40}(беспл|бесплатн\w*|даром|халяв)"
    r"|(дарит|раздае|раздаёт|раздают)\w*[^\n]{0,20}(роз|цвет|букет)",
    re.I,
)
